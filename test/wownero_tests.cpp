...
void synchronize_multisig_participants(vector<wownero_wallet*> wallets) {
  ...
  for (int i = 0; i < wallets.size(); i++) {
    ...
    wownero_wallet* wallet = wallets[i];
    wallet->sync();  // TODO wownero: creating multisig tx fails if wallet not explicitly synced before import_multisig_hex: https://github.com/wownero/wownero/issues/6850
...
void init_multisig_wallets(wownero_wallet* funding_wallet, vector<wownero_wallet*> participants, int M, int N, int account_idx, int num_addresses_to_fund) {
  ...
  for (int i = 0; i < participants.size(); i++) {
    ...
    // } catch (WowneroError e) {
    //   assertEquals("basic_string", e.getMessage()); // TODO (wownero): improve error message https://github.com/wownero/wownero/issues/8493
    // }
  ...
  // participant->close();
  // delete participant;
  // participant = createWallet(new WowneroWalletConfig().setSeed(seed).setIsMultisig(true));
  // WowneroUtils.validateAddress(participant.getPrimaryAddress(), TestUtils.NETWORK_TYPE);
  ...
  // try {
  //   participant.createTxs(new WowneroTxConfig().setAccountIndex(accountIdx).setAddress(returnAddress).setAmount(TestUtils.MAX_FEE.multiply(BigInteger.valueOf(3))));
  ...
  // try {
  //   participant.createTx(new WowneroTxConfig().setAddress(returnAddress).setAmount(TestUtils.MAX_FEE).setAccountIndex(accountIdx).setSubaddressIndex(0).setRelay(true));
  //   throw new RuntimeException("Should have failed");
  // } catch (Exception e) {
  //   assertTrue(e instanceof WowneroError);
